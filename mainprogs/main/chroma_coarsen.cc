...
int main(int argc, char *argv[]) 
{
  ...
  multi1d<int> nrow_f = 2*input.param.nrow;
  QDPIO::cout << "Fine lattice size:";
  for (int mu=0; mu<Nd; ++mu) QDPIO::cout << " " << nrow_f[mu];
  QDPIO::cout << endl;

  multi1d<int> nrow_c = input.param.nrow;
  QDPIO::cout << "Coarse lattice size:";
  for (int mu=0; mu<Nd; ++mu) QDPIO::cout << " " << nrow_c[mu];
  QDPIO::cout << endl;

  QDPIO::cout << "Creating fine lattice layout" << endl;
  Layout::setLattSize(nrow_f);
  Layout::create();

  proginfo(xml_out);    // Print out basic program info

  int sites_on_node_f = Layout::sitesOnNode();
  multi1d< multi1d<Int> > coord_f(Nd);
  multi1d< multi1d<ColorMatrix> > u_f(Nd);
  ...
  XMLBufferWriter config_xml;
  {
    ...
    QDPIO::cout << "Extracting fine lattice data" << endl;
    for (int mu=0; mu<Nd; ++mu) {
      tmp = u[mu] * shift(u[mu], FORWARD, mu); // Create product links
      coord_f[mu].resize(sites_on_node_f);
      u_f[mu].resize(sites_on_node_f);
      QDP_extract(u_f[mu],tmp,all);
      QDP_extract(coord_f[mu],Layout::latticeCoordinate(mu),all);
    }

  }

  QDPIO::cout << "Destroying fine lattice layout" << endl;
  Layout::destroy();

  QDPIO::cout << "Creating coarse lattice layout" << endl;
  Layout::setLattSize(nrow_c);
  Layout::create();

  QDPIO::cout << "Populating coarse lattice" << endl;
  multi1d<LatticeColorMatrix> u(Nd);
  for (int mu=0; mu<Nd; ++mu) { u[mu] = 1.0; }
  multi1d<int> coord_c(Nd);
  bool populateQ;
  for (int i=0; i<sites_on_node_f; ++i) {
    populateQ = true;
    for (int mu=0; mu<Nd; ++mu) {
      if ( (toInt( coord_f[mu][i] ) % 2) != 0) populateQ = false;
    }
    if (populateQ == true) {
      for (int mu=0; mu<Nd; ++mu) {
        coord_c[mu] = toInt( coord_f[mu][i])/2;
      }
      for (int mu=0; mu<Nd; ++mu) {
        pokeSite(u[mu],u_f[mu][i], coord_c);
      }
    }
  }

  QDPIO::cout << "Begin Chroma measurements" << endl;
