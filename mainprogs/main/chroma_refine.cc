...
int main(int argc, char *argv[]) 
{
  ...
  multi1d<int> nrow_f = input.param.nrow;
  ...
  for (int mu=0; mu<Nd; ++mu) {
    if (nrow_f[mu]%2!=0) {
      QDPIO::cerr << "CHROMA: fine lattice dimensions must be divisible by two" << endl;
      QDP_abort(1);
    }
  }

  multi1d<int> nrow_c = input.param.nrow/2;
  ...
  int sites_on_node_c = Layout::sitesOnNode();
  multi1d< multi1d<Int> > coord_c(Nd);
  multi1d< multi1d<ColorMatrix> > u_c(Nd);
  ...
  XMLBufferWriter config_xml;
  {
    ...
    QDPIO::cout << "Extracting coarse lattice data" << endl;
    for (int mu=0; mu<Nd; ++mu) {
      coord_c[mu].resize(sites_on_node_c);
      u_c[mu].resize(sites_on_node_c);
      QDP_extract(u_c[mu],u[mu],all);
      QDP_extract(coord_c[mu],Layout::latticeCoordinate(mu),all);
    }

  }

  QDPIO::cout << "Destroying coarse lattice layout" << endl;
  Layout::destroy();
  ...
  QDPIO::cout << "Populating fine lattice" << endl;
  multi1d<LatticeColorMatrix> u(Nd);
  for (int mu=0; mu<Nd; ++mu) { u[mu] = 1.0; }
  multi1d<int> coord_f(Nd);
  for (int i=0; i<sites_on_node_c; ++i) {
    for (int mu=0; mu<Nd; ++mu) {
      coord_f[mu] = 2*toInt( coord_c[mu][i]);
    }
    for (int mu=0; mu<Nd; ++mu) {
      pokeSite(u[mu],u_c[mu][i], coord_f);
    }
  }
