...
#include "inline_wilson_flow_adapt.h"
...
{ 
  ...
  namespace InlineWilsonFlowAdaptEnv 
  {
    ...
    {
      InlineWilsonFlowAdaptEnv::Params tmp(xml, path);
      input = tmp;
    }
  ...
  namespace InlineWilsonFlowAdaptEnv 
  {
    ...
    const std::string name = "WILSON_FLOW_ADAPT";
	...
	write(file_xml, "WilsonFlowAdaptParams", params.param);
	pop(file_xml);
